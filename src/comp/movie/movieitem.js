...
export default class Movieitem extends Component {
  render() {
    const { id, title, description, imgurl, rating } = this.props;
    const formattedRating = parseFloat(rating).toFixed(1);

    return (
      <div className="my-3">
        <div className="card" style={{ width: "18rem", border: 'none' }}>
          <img height={450} src={`https://image.tmdb.org/t/p/w500${imgurl}`} className="card-img-top" alt="..." />
          <div className="card-body" style={{ backgroundColor: '#282828' }}>
            ...
            <p style={{ height: '80px', overflow: 'hidden', color: 'white' }} className="card-text">{description}...</p>
